...
local module = {
    ...
    commands = {"lang","map", "profile", "p", "help", "h"},
    names = {},
    minium_player = 4 ,
    max_player = 30 , 
    started = false , 
...
local langue = {
    ...
    fr = {
        ...
        welcomeText = "<J><B>Bienvenue a <V>Hungry Mice</V>!</J></B>\n<J>Dans ce module, tout ce que vous avez à faire est de manger les pommes pour grossir</J>\n<J>Vous pouvez manger des souris plus petites que vous en appuyant sur<ROSE>[ESPACE]</ROSE> bouton</J>\n\n<BL>Ce module a été programmé par: <FC>%s</FC></BL>",
        canEatMice = "<%s>Les souris peuvent maintenant manger d'autres souris!</%s>",
        wonRound = "<V>%s</V> a remporté la manche!",
        eatPlayer = "<V>%s</V> a été mangé par <V>%s</V>.",
        poisonedApple = "<R>Vous avez mangé une pomme pourrie!</R>",
        diedPoisoned = "<V>%s</V> <R>est mort empoisonné.</R>",
        maxSize = "<V>%s</V> <J>atteint la taille maximale!</J>",
        changedLang = "<J>La langue a bien changé.</J>",
...
local foodsTypes = {
    ...
    [7] = {7, "177d0eab2dc.png", 0, -20 , -8, chance = {97, 100}},
}
...
function setColorName(name)
    if name == "Noooooooorr#0000" then return "0xFF8547" end
...
function eventNewPlayer(name)
    ...
    imgs[name] = {cheesemenu = nil, cheese = nil, sizeBanner = nil, profile = {}, helpMenu = {}}
    if not data[name] then 
        data[name] = {xp = 0, level = 1, avatar = 1, rounds = 0, miceEats = 0, wins = 0}
    end
    updateSize(name,p[name].size)
...
function eventChatCommand(name, mes)
    ...
    if cmd[1] == "lang" then
        if cmd[2] then
            if langue[cmd[2]] then
                ...
                TFM.chatMessage(string.format(translate("translatedBy",name), string.name(translate("translator",name))), name)
            else
    ...
    elseif cmd[1] == "map" then
        if name == module.stuff.developer then 
            setMap()
        end
